...
Time::Time(int min, int sec) { normalize_time(); }
...
fstream& operator >>(fstream& fin, Time& other)
{
	fin >> other.min;
	fin.ignore();// Пропускает один символ (двоеточие)
	fin >> other.sec;
	other.normalize_time();
	return fin;
}
fstream& operator <<(fstream& fout, const Time& other)
{
	fout << setw(2) << setfill('0') << other.min
		<< ":"
		<< setw(2) << setfill('0') << other.sec;
	return fout;
}
